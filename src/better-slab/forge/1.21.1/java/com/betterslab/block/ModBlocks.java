...
public class ModBlocks {
    ...
    public static final DeferredRegister<Block> BLOCKS = DeferredRegister.create(Registries.BLOCK, BetterSlab.MOD_ID);
    public static final DeferredRegister<Item> ITEMS = DeferredRegister.create(Registries.ITEM, BetterSlab.MOD_ID);

    /** 原版半砖 → 专属竖半砖的 DeferredHolder（仅原版半砖有）。 */
    private static final Map<Block, DeferredHolder<Block, Block>> VANILLA_TO_VERTICAL_RO = new HashMap<>();
    /** 反向缓存：专属竖半砖 → 原版半砖（运行时惰性构建）。 */
    private static Map<Block, Block> verticalToVanillaCache;

    /** 通用竖半砖方块（用于模组半砖，运行时存储来源半砖外观）。 */
    public static final DeferredHolder<Block, Block> GENERIC_VERTICAL_SLAB_RO = BLOCKS.register("generic_vertical_slab",
            () -> new GenericVerticalSlabBlock(BlockBehaviour.Properties.ofFullCopy(Blocks.STONE).noOcclusion()));

    /** 合并半砖方块（不同种类半砖合并到同一格，由 BlockEntity 记录两种来源）。 */
    public static final DeferredHolder<Block, Block> MERGED_SLAB_RO = BLOCKS.register("merged_slab",
            () -> new MergedSlabBlock(BlockBehaviour.Properties.ofFullCopy(Blocks.STONE).noOcclusion()));
    ...
    public static void register(IEventBus bus) {
        ...
        for (Block vanillaSlab : VANILLA_SLABS) {
            ResourceLocation vanillaId = BuiltInRegistries.BLOCK.getKey(vanillaSlab);
            ...
            DeferredHolder<Block, Block> ro = BLOCKS.register(name,
                    () -> new VerticalSlabBlock(BlockBehaviour.Properties.ofFullCopy(vanillaSlab)));
    ...
    public static Block getVerticalSlab(Block vanillaSlab) {
        DeferredHolder<Block, Block> ro = VANILLA_TO_VERTICAL_RO.get(vanillaSlab);
    ...
    public static Block getVanillaSlab(Block verticalSlab) {
        if (verticalToVanillaCache == null) {
            ...
            for (Map.Entry<Block, DeferredHolder<Block, Block>> e : VANILLA_TO_VERTICAL_RO.entrySet()) {
