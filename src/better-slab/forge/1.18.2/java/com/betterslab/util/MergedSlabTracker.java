...
public class MergedSlabTracker {
    ...
    private static String key(Level world, BlockPos pos) {
        return world.dimension().location().toString() + ":" + pos.getX() + "," + pos.getY() + "," + pos.getZ();
    }

    public static void setMerged(Level world, BlockPos pos, Block secondSlab, SlabType secondType) {
    ...
    /** 兼容旧调用：默认层位为 TOP。 */
    public static void setMerged(Level world, BlockPos pos, Block secondSlab) {
        setMerged(world, pos, secondSlab, SlabType.TOP);
    }

    public static MergedEntry getMerged(Level world, BlockPos pos) {
    ...
    public static boolean hasMerged(Level world, BlockPos pos) {
        return mergedData.containsKey(key(world, pos));
    }

    /** 检查 pos 处是否为新系统的 MergedSlabBlock（自带 BlockEntity 持久化的合并半砖）。 */
    public static boolean isMergedSlabBlock(Level world, BlockPos pos) {
