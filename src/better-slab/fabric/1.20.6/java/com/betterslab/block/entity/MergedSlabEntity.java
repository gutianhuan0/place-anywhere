...
public class MergedSlabEntity extends BlockEntity {
    ...
    protected void readNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
        ...
        if (!strA.isEmpty()) {
            slabA = Registries.BLOCK.getOrEmpty(new Identifier(strA)).orElse(Blocks.STONE_SLAB);
        }
        if (!strB.isEmpty()) {
            slabB = Registries.BLOCK.getOrEmpty(new Identifier(strB)).orElse(Blocks.STONE_SLAB);
        }
    }
