...
public final class FreePlacementMode {
    ...
    public static void initKeys() {
        ...
        // 1.18.2: KeyMapping 在构造函数中自动注册，无需 event.register()
    }
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        while (axisKey.consumeClick()) {
            ...
            mc.player.displayClientMessage(new TextComponent("§e旋转轴: " + axisName), true);
        }

        // PageUp/PageDown 调整 Y 偏移
        while (upKey.consumeClick()) {
            yOffset += 0.5f;
            mc.player.displayClientMessage(new TextComponent("§eY偏移: " + yOffset), true);
        }
        ...
        while (resetKey.consumeClick()) {
            resetTransform();
            mc.player.displayClientMessage(new TextComponent("§e已重置旋转和Y偏移"), true);
        }
    }
    ...
    private static boolean isOverlapping(Minecraft mc, Vec3 pos, Quaternionf q) {
        ...
        AABB obbBoundingBox = FreeBlocks.rotateBoxAABB(
                ...
                q.x, q.y, q.z, q.w);
        BlockPos minBp = new BlockPos(obbBoundingBox.minX, obbBoundingBox.minY, obbBoundingBox.minZ);
        BlockPos maxBp = new BlockPos(obbBoundingBox.maxX, obbBoundingBox.maxY, obbBoundingBox.maxZ);
    ...
    public static boolean tryPlace() {
        ...
        if (isOverlapping(mc, pos, q)) {
            mc.player.displayClientMessage(new TextComponent("§c无法放置：与现有方块重叠"), true);
            return true; // 拦截原版放置，但不发送放置包
        }
        FreeBlockNetworking.INSTANCE.sendToServer(new FreeBlockInteractPayload(
    ...
    public static void onRenderLevelStage(RenderLevelStageEvent event) {
        ...
        VoxelShape collision = state.getCollisionShape(mc.level, new BlockPos(pos.x, pos.y, pos.z));
    ...
    /** 画 AABB 的 12 条边线。 */
    private static void drawBoxLines(VertexConsumer lines, com.mojang.math.Matrix4f pose,
                                      double minX, double minY, double minZ,
                                      double maxX, double maxY, double maxZ,
                                      float r, float g, float b, float a) {
        double[][] p = {
