# Place Anywhere Free Placement / 自由放置模式

Free placement mode — toggle with G, rotate with V + mouse wheel.

自由放置模式 —— G键切换，V键旋转轴+鼠标滚轮调整角度。

---

## Branch: `fabric-1.21.1` / 分支：`fabric-1.21.1`

| | |
|---|---|
| **Loader / 加载器** | Fabric / Fabric |
| **Minecraft Version / MC 版本** | 1.21.1 |
| **Java Version / Java 版本** | 21 |
| **Status / 状态** | **Active development** / **活跃开发中** |


## Dependencies / 依赖

- **Place Anywhere Core** (required) — build the same version first / 需先构建对应版本的 Core

> Uncomment the `modCompileOnly` line in `build.gradle` and point it to the core JAR.
> 取消 `build.gradle` 中 `modCompileOnly` 行的注释，指向 core 的 JAR 文件。
